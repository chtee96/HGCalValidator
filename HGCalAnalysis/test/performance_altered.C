...
void setTDRStyle();
shower_reco_perf getPerformance(std::vector<TTree*> trees_, std::vector<TTree*> treesresponse_, std::vector<unsigned int> energies_, TString var_, int color_,TString suffix, TString region, TString cut="0==0");
...
void performance(std::string inputFileTemplate = "/eos/cms/store/group/dpg_hgcal/comm_hgcal/apsallid/CalibrationStudies/ResolutionTrees/hgc_singlephoton_eENERGYGeV_nopu.root", std::string region = "CE_E_Front_120um") {
  //void performance(std::string inputFileTemplate = "hgc_singlephoton_nopu.root", std::string region = "CE_E_Front_120um") {
  ...
  // std::vector<std::string> inputTreeTemplates; 
  // inputTreeTemplates.push_back("ttree_reconstructable_energy_eENERGY");
  // inputTreeTemplates.push_back("ttree_uncalib_energy_eENERGY");
  std::string theresp = "ttree_SumEoverEgen_eENERGY";
  //inputTreeTemplates.push_back("ttree_SumEoverEgen_eENERGY");

  std::vector<TTree*> trees_recostr_nopu_;
  std::vector<TTree*> trees_uncalib_nopu_;
  std::vector<TTree*> trees_response_nopu_;
  for (unsigned int itree = 0; itree<energies_.size(); ++itree) {
    ...
    std::string inputTree = "ttree_reconstructable_energy_eENERGY";
    inputTree = replaceFirstOccurrence(inputTree, "ENERGY", to_string(energies_[itree]));
    TTree *trecostr_ = (TTree*)f_->Get(Form("%s_%s",inputTree.c_str(),region.c_str()));
    inputTree = "ttree_uncalib_energy_eENERGY";
    inputTree = replaceFirstOccurrence(inputTree, "ENERGY", to_string(energies_[itree]));
    TTree *tuncalib_ = (TTree*)f_->Get(Form("%s_%s",inputTree.c_str(),region.c_str()));
    std::string inputTreeResp = replaceFirstOccurrence(theresp, "ENERGY", to_string(energies_[itree]));
    TTree *tresp_ = (TTree*)f_->Get(Form("%s_%s",inputTreeResp.c_str(),region.c_str()));
    
    trees_recostr_nopu_.push_back(trecostr_);
    trees_uncalib_nopu_.push_back(tuncalib_);
    trees_response_nopu_.push_back(tresp_);
  
  }
  std::cout << " performance: nopu \n";
  shower_reco_perf perf_recable_nopu        = getPerformance(trees_recostr_nopu_,trees_response_nopu_,energies_,"reconstructable_energy_" + region,4,"erecable", region);
  std::cout << " performance: recable \n";
  shower_reco_perf perf_uncalib_nopu     = getPerformance(trees_uncalib_nopu_,trees_response_nopu_,energies_,"uncalib_energy_" + region,1,"euncalib", region);
...
//
  ...
  perf_recable_nopu.h_resolution->Draw("AP");
  perf_recable_nopu.h_resolution->GetYaxis()->SetRangeUser(0.,0.5);
  perf_recable_nopu.h_resolution->GetYaxis()->SetTitle("#sigma(E_{RECO}) / <E_{RECO}>");
  perf_recable_nopu.h_resolution->GetXaxis()->SetTitle("E_{GEN}");
  perf_uncalib_nopu.h_resolution->Draw("P sames");
  ...
  perf_recable_nopu.h_resolution_corr->Draw("AP");
  perf_recable_nopu.h_resolution_corr->GetYaxis()->SetRangeUser(0.,0.5);
  perf_recable_nopu.h_resolution_corr->GetYaxis()->SetTitle("#sigma(E_{RECO}) / <E_{RECO}>");
  perf_recable_nopu.h_resolution_corr->GetXaxis()->SetTitle("E_{GEN}");
  perf_uncalib_nopu.h_resolution_corr->Draw("P sames");
...
shower_reco_perf getPerformance(std::vector<TTree*> trees_, std::vector<TTree*> treesresponse_, std::vector<unsigned int> energies_, TString var_, int color_, TString suffix,TString region, TString cut="0==0") {
  ...
  for (unsigned int itree = 0; itree<trees_.size(); ++itree) {
    ...
    TH1F *h_en = new TH1F("h_"+nameEn,"h_"+nameEn,100,e[itree]-e[itree]*0.5, e[itree]+e[itree]*0.5);
...
//     //TString var_resp = "SumEoverEgen_e"+ e[itree] + "_" + region; std::cout << " var_resp = " << var_resp << "\n";
//     treesresponse_[itree]->Project("h_"+name,"SumEoverEgen_"+region,cut);
